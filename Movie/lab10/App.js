...
app.get("/addstudent", (req, res) => {
    ...
    // Sanitization XSS...
    let qry = "select * from movies where email=? or phone=?";
    mysql.query(qry, [email, phone], (err, results) => {
        if (err)
            ...
            if (results.length > 0) {
                ...
                // insert query
                let qry2 = "insert into movies values(?,?,?,?)";
...
app.get("/searchstudent", (req, res) => {
    ...
    let qry = "select * from movies where phone=?";
...
app.get("/updatestudent", (req, res) => {
    ...
    let qry = "update movies set username=?, gender=? where phone=?";
...
app.get("/removestudent", (req, res) => {
    ...
    let qry = "delete from movies where phone=?";
    mysql.query(qry, [phone], (err, results) => {
        if (err) throw err
        else {
            if (results.affectedRows > 0) {
